...
if (isset($_POST['submit'])) {
    ...
    if ($password == $cpassword) {
        ...
        if (!$result->num_rows > 0) {
            $sql = "INSERT INTO data_user (username, email, password, fname, lname)
                    VALUES ('$username', '$email', '$password', '$fname', '$lname')";
            $result = mysqli_query($conn, $sql);
            if ($result) {

                // $sql = "INSERT INTO data_pajak (username) VALUES ('$username')";
                // $result = mysqli_query($conn, $sql);
...
<head>
    ...
    <title>JakPajak</title>
</head>
<body>
    <center>
    <div class="container">
        <form action="" method="POST" class="loginbox" id="form1">
